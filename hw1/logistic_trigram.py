	...
	def train(self):
		...
		for i in range(1,16):
			...
			for line in self.train_file:	#d_i
				...
				# trigram
				if len(line_fill[1]) > 2:
					new_doc = []
					firstWord = line_fill[1][0]
					secondWord = line_fill[1][1]
					for word in line_fill[1][2:]:
						trigram = firstWord + ' ' + secondWord + ' ' + word
						new_doc.append(trigram)
						firstWord = secondWord
						secondWord = word
					p_k_given_d_trigram = self.p_k_given_d_speakers(new_doc)
					for val in p_k_given_d:
						p_k_given_d[val] += p_k_given_d_trigram[val]
					for word in (new_doc + [""]):
						if word in self.lamb_doll_word[category]:
							self.lamb_doll_word[category][word] = self.lamb_doll_word[category][word] + eta
						else:
